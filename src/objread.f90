Module obj_reader
    ...
    Contains
        ...
        subroutine read_obj(filename, tarray, id, fit)
            ...
            character(len=256) :: line, file

            file = trim(resdir)//trim(filename)

            if(id == 0)print*,'Reading: ',trim(file)
            open(newunit=u,file=file,iostat=io)
            ...
            call read_vert(file, varray)
            call read_faces(file, farray)
            call read_texture_coor(file, textarray)
            call read_vert_normals(file, narray)
        ...
        subroutine make_triangle(varray, farray, tarray, textarray, narray)
            ...
            do i = 1, size(farray,1)
                ...
                tarray(i) = triangle(tmp, tmp2, tmp3)
            end do
